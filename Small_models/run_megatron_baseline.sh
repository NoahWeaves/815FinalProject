...
# Megatron-DeepSpeed Baseline (No Pipeline Parallelism)
# Comparison baseline using DDP with ZeRO Stage 0
...
# Model configuration
TP=1              # Tensor parallelism (disabled for baseline)
PP=1              # Pipeline parallelism (disabled - this is baseline)
...
SEQ_LENGTH=512    # Sequence length (shorter for faster training)

# DeepSpeed configuration
ZERO_STAGE=0      # ZeRO stage 0 (no ZeRO optimization - pure DDP)

# Paths
BASE_PATH=$workdir/gpt
DATA_PATH=$BASE_PATH/my-gpt2_text_document
DS_CONFIG=$workdir/ds_config_baseline.json
HOST_FILE=$workdir/hostfile
OUTPUT_DIR=$workdir/output/baseline_pp${PP}_nl${NLAYERS}_gb${GLOBAL_BATCH}
...
echo "="*80
echo "Megatron-DeepSpeed Baseline Training"
echo "="*80
echo "Configuration:"
echo "  Model: GPT-2 (${NLAYERS} layers, ${HIDDEN} hidden)"
echo "  Pipeline stages: ${PP} (baseline - no pipeline)"
